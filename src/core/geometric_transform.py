...
def bbox2points(box):
    ...
    return torch.cat([cx, min_y, max_x, cy, cx, max_y, min_x, cy], dim=1).reshape(
        -1, 2
    )  # n*4,2
...
def warp_affine(
    ...
    src_norm_trans_dst_norm = torch.inverse(dst_norm_trans_src_norm.float()).to(
        dst_norm_trans_src_norm.dtype
    )
...
class Transform2D:
    ...
    @staticmethod
    def transform_image(img, M, out_shape, mode="nearest"):
        if isinstance(img, Sequence):
            assert len(img) == len(M)
            return [
                Transform2D.transform_image(b, m, shape, mode)
            ...
            return warp_affine(
                img.float(), M[None, ...], out_shape, mode=mode
            ).squeeze()


def filter_invalid(bbox, label, score=None, mask=None, thr=0.0, min_size=0):
...
def recover_mask(bitmask, meta, scale=1.0):
    pad_mask = F.interpolate(bitmask[None, None, ...].float(), scale_factor=scale)
    ih, iw = meta["img_shape"][:2]
    mask = pad_mask[:, :, :ih, :iw]
    return F.interpolate(mask, meta["ori_shape"][:2])[0, 0].long(), None

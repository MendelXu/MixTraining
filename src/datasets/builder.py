...
from mmdet.datasets import build_dataset as _base_dataset
...
try:
    from .dali_loader import DaliDataLoader
except:
    DaliDataLoader = None
...
def build_dataset(cfg, default_args=None):
    extension = cfg.pop("dali_extension", {})
    dataset = _base_dataset(cfg, default_args=default_args)
    for name, ext in extension.items():
        setattr(dataset, name, ext)
    return dataset
...
def build_dataloader(
    ...
    **kwargs,
    ...
    if loader_cfg is None:
        loader_cfg = dict()
    loader_mode = loader_cfg.get("mode", "normal")
    if loader_mode == "normal":
        init_fn = (
        ...
        data_loader = DaliDataLoader(
            ...
            sampler=sampler,
            num_workers=num_workers,
            pipeline_cfg=dataset.pipeline_cfg,
            **kwargs,
        )
    return data_loader

...
class ImShear(object):
    ...
    def __call__(self, results):
        ...
        if np.random.rand() > self.prob:
            return results
        self.level = np.random.randint(1, self.max_level)
        self.magnitude = level_to_value(self.level, self.max_shear_magnitude)
        ...
        self._shear_masks(
            results, magnitude, self.direction, fill_val=0, interpolation="nearest"
        )
        self._shear_seg(
            results,
            magnitude,
...
class ImRotate(object):
    ...
    def __call__(self, results):
        ...
        self.level = np.random.randint(1, self.max_level)
        self.angle = level_to_value(self.level, self.max_rotate_angle)
...
class ImTranslate(object):
    ...
    def __call__(self, results):
        ...
        if np.random.rand() > self.prob:
            return results
        self.level = np.random.randint(1, self.max_level)
        self.offset = int(level_to_value(self.level, self.max_translate_offset))

...
def load_state_dict_from_url(
    ...
    if not os.path.exists(cached_file):
        ...
        hash_prefix = hub.HASH_REGEX.search(filename).group(1) if check_hash else None
        hub.download_url_to_file(url, cached_file, hash_prefix, progress=progress)
...
def load_url_dist(url, model_dir=None):
    ...
    if rank == 0:
        checkpoint = load_state_dict_from_url(url, model_dir=model_dir)
    if world_size > 1:
...
def find_latest_checkpoint(path):
    ...
    for checkpoint in checkpoints:
        epoch = int(os.path.basename(checkpoint).split("_")[-1].split(".")[0])
        if epoch > latest:
            latest = epoch
